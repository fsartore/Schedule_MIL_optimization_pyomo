...
<div style="border: 2px solid #4CAF50; padding: 15px; background-color: #f9f9f9; border-radius: 5px;">
  ...
  <p>The <code>Bounds.partition_dict</code> function is designed to divide the original optimization problem in multiple subproblems. In every subproblem just a fraction of the total number of units' tasks will be optimized. After the first optimization, the following will take into account the previous solutions to avoid overlaps. 
  This function is a jey part of this optimization framework, as it allows to reach a solution using commercial solvers. Clearly, the final solution cannot be as gooos as the one obtained looking at the whole problem.</p>
