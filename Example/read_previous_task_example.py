...
class ReadLasttask:
    ...
    def read_last_task(self):
        ...
        '''       
        self.last_task_df = pd.read_excel(f'{self.data_path}/last_task_data.xlsx', sheet_name='Previous Task')
        self.last_task_df['end_date'] = pd.to_datetime(self.last_task_df['end_date'])
        last_task_dict_time = {worker: None for worker in self.unit_names}
        last_task_dict_year = {worker: None for worker in self.unit_names}

        for worker in self.unit_names:

            df_worker = self.last_task_df[self.last_task_df['Workers'] == worker]
            last_task_time = df_worker['end_date'].values[0]

            # Store the result in the dictionary
            last_task_dict_time[worker] = last_task_time
            
        # CONVERT last task time to timestamp
        last_task_dict_time = {worker: pd.Timestamp(last_task_dict_time[worker]) for worker in self.unit_names}
        last_task_dict_year = {worker: pd.Timestamp(last_task_dict_time[worker]).year for worker in self.unit_names}
