...
class TimeHandling:
    ...
    def generate_holidays_days_list(self):
        ...
        for year in self.current_year_list:
            file_path = os.path.join(self.file_path_input, f"holidays.xlsx")
            df_hol = pd.read_excel(file_path, sheet_name='Holidays')
    ...
    def weekend_set(self, start_date, time_horizon_tot):   # inttype
        weekday_of_start_date = start_date.weekday()
    ...
    def modify_task_schedule(self,check_overlap,  m, days_to_monday, counter,storage_x,  start_end_time_dict, active_gen_keys_list, counter_overlap,counter_overlap_max, maintenance_days):
        ...
        days_to_monday = days to the first monday from the first day
        start_end_time_dict = start end of maintenance for each generators
        active_gen_key_list = gen in this partition to be possibly changed
        counters = counters of various unfiseability
        '''
        ...
        #Initiate
        first_day_prev = [0 for _ in range(len(active_gen_keys_list))]
        last_day_prev = [0 for _ in range(len(active_gen_keys_list))]
        man_per_prev_monday_prev = [0 for _ in range(len(active_gen_keys_list))]
        first_day_next = [0 for _ in range(len(active_gen_keys_list))]
        last_day_next = [0 for _ in range(len(active_gen_keys_list))]
        man_per_next_monday_next = [0 for _ in range(len(active_gen_keys_list))]


        for index, i in enumerate(active_gen_keys_list):
            ...
            man_per_prev_monday_prev[index]= (last_day_prev[index]+1 - first_day_prev[index])
            man_per_next_monday_next[index] = (last_day_prev[index]+1 - first_day_prev[index])
            ...
            2) the maintenance days are not consecutive
            ...
            if first_day % 7 != days_to_monday + 1 or sum(pyomo.value(m.x[i,j]) for j in range(first_day, last_day + 1) if j not in m.weekends and j not in m.holidays_day) <= (last_day + 1 - first_day - total - 0.1) or any(index in all_busy_days_prev for index in non_zero_indices_origin):          
                ...
                # ensure the solution has n_maintenance solution days for prev solution
                non_zero_indices_prev = [i for i, x in enumerate(specific_x_prev[index]) if x == 1.0]
                if len(non_zero_indices_prev) > maintenance_days:
                    for it in range(maintenance_days, len(non_zero_indices_prev)):
                        specific_x_prev[index][non_zero_indices_prev[it]] = 0.0
                    non_zero_indices_prev = [i for i, x in enumerate(specific_x_prev[index]) if x == 1.0]    

                # ensure the solution has n_maintenance solution days for next solution
                non_zero_indices_next = [iti for iti, x in enumerate(specific_x_next[index]) if x == 1.0]
                if len(non_zero_indices_next) > maintenance_days:
                    for itit in range(maintenance_days, len(non_zero_indices_next)):
                        specific_x_next[index][non_zero_indices_next[itit]] = 0.0
                    non_zero_indices_next = [iti for iti, x in enumerate(specific_x_next[index]) if x == 1.0]        
                elif len(non_zero_indices_next)<maintenance_days:
                    specific_x_next[index] = specific_x_prev[index]
                    non_zero_indices_next = [iti for iti, x in enumerate(specific_x_next[index]) if x == 1.0]
                ...
                man_per_next_monday_next[index] = (last_day_next[index]+1 - first_day_next[index])
    
                '''Update specific_x dpepending on the quality of prev, next solution
                CRITERIAS:
                1) favour the solution with less maintenance days to improve the availability
                2) avoid overlaps
                ACTIONS:
                1) If all good update specific_x
                2) Raise a value error to activate unfeasible tricks to distrivute better the target times
                3) If close to the last unfeasible tricks, accept an overlap solution
                '''
                if index == 0 and counter == 0:
                    specific_x[index] = specific_x_prev[index] if man_per_prev_monday_prev[index]<=man_per_next_monday_next[index] else specific_x_next[index]
                else:
                    if man_per_prev_monday_prev[index] <= man_per_next_monday_next[index] :                        
                        if not any(index in all_busy_days_index for index in non_zero_indices_prev):
                            ...
                            if counter_overlap<=counter_overlap_max-1 and check_overlap == True:
                                print(f"\033[94mNo overlap could be solved for generator {i}\033[0m")
                                raise ValueError(f"No averlap could be solved for generator {i}")
                            else:
                                specific_x[index] = specific_x_prev[index]                         
                    else:
            ...
            start_time = 0
            end_time = 0
            # start_end_time_dict = {index: (0,0) for index in active_gen_keys_list}
